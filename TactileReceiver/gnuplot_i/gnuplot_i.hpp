...
//defined for 32 and 64-bit environments
 ...
 #define GP_MAX_TMP_FILES  6400000   // 27 temporary files it's Microsoft restriction
#elif defined(unix) || defined(__unix) || defined(__unix__) || defined(__APPLE__) 
//all UNIX-like OSs (Linux, *BSD, MacOSX, Solaris, ...)
 #include <unistd.h>            // for access(), mkstemp()
 #define GP_MAX_TMP_FILES  6400000
#else
...
class Gnuplot
{
    ...
    ///saves a gnuplot session to a png file, filename without estension
    ...
    template <typename X>
    Gnuplot& plot_x_x(const X& x, const X& y, const std::string &title1 = "" , const std::string &title2 = "");

    Gnuplot& plotfile_x_x(const std::string &filename, const std::string &filename1,
    ...
    Gnuplot& plot_xxy(const X& x, const Y& y, const Z& z, 
        const std::string &title1 = "" , const std::string &title2 = "");

    Gnuplot& plotfile_xxy(const std::string &filename, const std::string &filename1,
                            const std::string &title1, const std::string &title2,
                      const unsigned int column_x = 1, const unsigned int column_y1 = 2);
...
std::string Gnuplot::m_sGNUPlotPath = "/usr/local/Cellar/gnuplot/5.0.0/bin/";
#endif
 ...
 */
template <typename X>
Gnuplot& Gnuplot::plot_x_x(const X& x, const X& y, const std::string &title1, const std::string &title2){
    if (x.size() == 0)
    {
        throw GnuplotException("std::vector too small");
        return *this;
    ...
    tmp1.flush();
    tmp1.close();
    plotfile_x_x(name,name1,title1,title2);
    return *this;
}
...
template<typename X, typename Y, typename Z>
Gnuplot& Gnuplot::plot_xxy(const X& x, const Y& y, const Z& z, 
        const std::string &title1, const std::string &title2)
{
    ...
    for (unsigned int i = 0; i < x.size(); i++)
        tmp << x[i] << " " << y[i] << std::endl;
    for (unsigned int i = 0; i < x.size(); i++)
        tmp1 << x[i] << " " << z[i] << std::endl;

    tmp.flush(); tmp1.flush();
    tmp.close(); tmp1.close();
    ...
    Gnuplot& Gnuplot::plotfile_xxy(const std::string &filename, const std::string &filename1,
                const std::string &title1, const std::string &title2, 
                      const unsigned int column_x, const unsigned int column_y1)
{
    //
...
/* MODIFICA */
Gnuplot& Gnuplot::plotfile_x_x(const std::string &filename,const std::string &filename1,
                    const std::string &title1 = "",const std::string &title2 = "" ){
    file_available(filename);
    file_available(filename1);
    ...
    cmdstr << "'" << filename << "' ";//using 1:2";// << column;
...
//        cmdstr << "plot ";
    ...
    cmdstr << "'" << filename1 << "' ";//using 1:2";// << column;
